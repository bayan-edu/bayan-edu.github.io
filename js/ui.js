...
function loadMathJax(){
  ...
  return mjReady = new Promise((ok, no) => {
    ...
    window.MathJax = {
      ...
      /* رموز المنهج المصريّ — تُعرَّف مرّةً هنا ويُكتب المصدر بها.
         الإضافة مشرقية: ℝ عندها «ر» والمصريّ «ح»، وi تبقى لاتينية.
         ⇒ الترميز صفةُ منهجٍ يُعلَن في موضعٍ واحد، لا حرفٌ يتكرّر في ٥٤ بنداً.
         ولا أثر على اللاتينية: الماكرو تعريفٌ ساكن، وما لا يُنادى لا يُنفَّذ.
         و\text{} مقصودة — العربية لا تُمال، فالوضع القائم هو الصواب. */
      TeX: { Macros: {
        en: ['#1', 1],
        ii: '\\text{ت}',      // الوحدة التخيلية
        RR: '\\text{ح}',      // الأعداد الحقيقية
        CC: '\\text{ك}',      // الأعداد المركبة
        ZZ: '\\text{ص}'       // الأعداد الصحيحة
      } },
      AuthorInit(){
